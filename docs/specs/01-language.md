...
ExternFunDecl  ::= "extern" "fun" LOWER_IDENT "(" ParamList ")" ":" Type "=" "jvm" "(" STRING ")"
                 | "extern" "fun" LOWER_IDENT "<" TypeParamList ">" "(" ParamList ")" ":" Type "=" "jvm" "(" STRING ")"
...
- **Extern function** (`extern fun`): `extern fun f(...): T = jvm("Class#method(Args)")` binds a function name directly to a JVM static/instance/constructor target without a Kestrel body. When the JVM method returns a primitive type, an optional `:ReturnType` suffix encodes the JVM return descriptor: `extern fun f(...): Int = jvm("Class#method(Args):long")`. Supported return type names: `boolean`, `byte`, `char`, `short`, `int`, `long`, `float`, `double`, `void`. Without the suffix, the compiler derives a boxed JVM return type from the Kestrel return type annotation (correct for methods returning reference types such as `java.lang.String`). For `extern import`-generated stubs, the compiler automatically propagates the correct return type descriptor obtained from `javap` metadata.
...
- **Narrowing:** When `e` is a **simple identifier** `x`, the type of `x` is refined in **`if`**’s **then**-branch and **`while`**’s **body** to **`original_type & T`** (06 §4). This refinement is **branch-local only**: the **`else`** branch (if any) and outer scope keep **`x`** at the **unrefined** type. Standalone **`x is T`** does not change the binding’s type outside the boolean result.
...
Stmt           ::= "val" LOWER_IDENT "=" Expr
                 ...
                 | [ "async" ] "fun" LOWER_IDENT [ "<" TypeParamList ">" ] "(" ParamList ")" ":" Type "=" Expr
                 | Expr ":=" Expr
                 | "break"
                 | "continue"
```

A block normally ends with a trailing **Expr** as its value. Alternatively, the block may close with **`}`** immediately after the last **Stmt** (no trailing **Expr**); that is equivalent to a trailing **`()`** (type **Unit**, 06). The typechecker always sees a block with a normal **result** expression after this desugar.

A block-local **`fun`** declaration is **desugared** to `val name = (params) => body`; a block-local **`async fun`** is desugared to `val name = async (params) => body` and establishes a new async context for its body (see §5). See §3.8 for closures. When the nested `fun` has a **full type signature** (all parameter types and return type), the name is in scope for the body so the function may call itself recursively. Every such block-local `fun` is also in scope in the body of every other such `fun` in the same block, so they may call each other (mutual recursion); declaration order does not affect name resolution.

**Trailing expression vs implicit Unit:** A block’s value is determined by its final item, uniformly across all block positions (fun bodies, `match` case blocks, `if`/`else` branches, `try`/`catch` bodies, `while` bodies, and any nested block):

- If the block ends with a trailing **Expr** (or with a `BlockItem` whose value is an `ExprStmt` promoted to the result), that expression is the block’s value.
- If the block ends immediately after a **`val`**, **`var`**, **`fun`**, or **`:=`** statement, the block’s value is implicitly **`()`** (type **Unit**, 06); you need not write a trailing `()`.
- If the block ends immediately after **`break`** or **`continue`**, the block’s formal value is a **synthetic never** (a fresh type variable in inference, 06) that unifies with any expected type, since control never reaches the tail.

There is no distinction between “statement-oriented” and “expression-oriented” block positions for the purpose of implicit-Unit synthesis.

**`if` branches:** Branch blocks follow the same implicit-Unit rule as all other blocks: a branch block may end with `:=` / `val` / `var` / `fun` without a trailing expression — the branch's value is implicitly `()` (type **Unit**). This applies in all positions, including `if` used in an expression context such as `val y = if (c) { x := 1 } else { x := 2 }`. The type-checker verifies that the resulting branch types unify with the surrounding context (06).

Each statement is followed by a **statement terminator**: `;`, newline, or `}` (when the next token starts the block’s final expression, or when `}` closes the block with an implicit-Unit result as above). `NL` denotes a newline token (or newline codepoint) used as terminator. See §3.5.
...
Precedence: `&` tighter than `|`; `->` groups to the right; `*` (product) tighter than `->`. Record and function types use parentheses when nested (e.g. `(Int -> Bool) -> String`). **Built-in generics:** `Task<T>` is a runtime built-in. **`Array<T>`** is a mutable, O(1)-indexed sequence backed by `java.util.ArrayList`; it is exposed as the opaque type `Array<T>` in the `kestrel:data/array` stdlib module — see [02-stdlib.md](02-stdlib.md). **Library types:** `Option<T>`, `Result<T,E>`, and `List<T>` are provided by the standard library; see [02-stdlib.md](02-stdlib.md). List has special syntax: `[a, b, ...c]` and `::` (expression and pattern).
...
- **Record vs block:** `{` can start Block or RecordLit. After `{`, if the next token is `}` (empty record literal) or a label (IDENT `=` or `...`), parse as RecordLit; if the next token is `val`, `var`, `fun`, `async`, `break`, `continue`, or an expression start, parse as Block. (Expression start: Literal, IDENT, `(`, `[`, `throw`, `if`, `while`, `match`, `try`, `await`, etc.) Note: `async` before `fun` starts a block-local `async fun` statement (§3.3); `async` before `(` or `<` starts an async lambda expression (§5); both parse as Block. A Block may always end after the last **Stmt** without a trailing **Expr** (implicit **Unit** result, §3.3); this disambiguation concerns only the leading `{` token.
...
**Block-local `async fun`:** `async fun` is also valid at block scope (§3.3): `async fun helper(x: Int): Task<Int> = { ... }`. It is desugared to `val helper = async (x: Int) => ...`, establishes a new async context for its body (permitting `await`), and must be declared with return type `Task<T>`. Like other block-local `fun` declarations it may be recursive and mutually recursive. A non-`async` block-local `fun` nested inside an async function is still **not** an async context.

**Await expression:** The grammar allows `await` as an optional prefix on a primary expression: `await` *expr* (e.g. `await f()`). `await` is only valid in an **async context** (inside an `async fun` — top-level or block-local — or `async` lambda); use outside async is a semantic (type/context) error. A non-async lambda or non-async block-local `fun` nested inside an async function is still **not** an async context. The operand of `await` is typed via **unification** against `Task<A>`: if the operand type is an unresolved inference variable (as can arise with a direct recursive call before the callee's type has settled), the type checker constrains it to `Task<A>` for a fresh `A` rather than immediately rejecting it. This means `await f(args)` and `val t: Task<T> = f(args); await t` are semantically equivalent and both accepted (see §6 in [06-typesystem.md](../specs/06-typesystem.md)).
...
- Runtime stdlib I/O/process tasks use `Result` payloads for expected operational failures. For example, `await Fs.readText(path)` produces `Result<String, FsError>`, `await Fs.listDir(path)` produces `Result<List<String>, FsError>`, and `await Process.runProcess(program, args)` produces `Result<ProcessResult, ProcessError>` where `ProcessResult = { exitCode: Int, stdout: String }`; callers pattern-match on `Ok` / `Err`.
- Process lifetime at top-level return is controlled by the CLI run mode (09 §2.1): default `kestrel run` / `--exit-wait` waits for pending async tasks to quiesce before exit; `--exit-no-wait` exits when `main` returns and may interrupt in-flight virtual-thread work. The quiescence wait has a timeout (default 30 000 ms; configure via the `kestrel.exitWaitTimeoutMs` JVM system property, `0` = infinite). If the timeout expires with tasks still running, a warning is printed to stderr and the process exits with code 1.

**Task combinators (`kestrel:task`):** The stdlib module `kestrel:task` provides combinators:
- `Task.map(task, f)` — transform the result of a `Task<A>` with `f: A -> B`, returning a new `Task<B>` without blocking. Cancelling the returned task also cancels the source task.
- `Task.all(tasks)` — wait for all tasks in a `List<Task<T>>` to complete and collect results into `Task<List<T>>`; fails fast if any task fails. When one task fails the combined task completes exceptionally immediately, but the remaining tasks continue running (they are not cancelled).
- `Task.race(tasks)` — return the result of the first `Task<T>` in a `List<Task<T>>` to complete. After the first task completes (whether successfully or with an error), all still-running tasks are cancelled. If all tasks fail, the first failure propagates. Calling `race` with an empty list produces a failed task whose failure is a catchable Kestrel exception with payload `"no tasks provided"`.
- `Task.cancel(t)` — request cancellation of `t`. Calls `CompletableFuture.cancel(true)` — best-effort I/O interruption on blocked virtual threads. Cancelling an already-completed task is a no-op. Any `await`-ing callers receive `Cancelled`.
