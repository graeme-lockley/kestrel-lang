...
Precedence: `&` tighter than `|`; `->` groups to the right; `*` (product) tighter than `->`. Record and function types use parentheses when nested (e.g. `(Int -> Bool) -> String`). **Built-in generics:** `Task<T>` is a runtime built-in. **`Array<T>`** is a mutable, O(1)-indexed sequence backed by `java.util.ArrayList`; it is exposed as the opaque type `Array<T>` in the `kestrel:array` stdlib module — see [02-stdlib.md](02-stdlib.md). **Library types:** `Option<T>`, `Result<T,E>`, and `List<T>` are provided by the standard library; see [02-stdlib.md](02-stdlib.md). List has special syntax: `[a, b, ...c]` and `::` (expression and pattern).
