...
For self-hosting compiler work, `kestrel:dev/typecheck/types`, `kestrel:dev/typecheck/from-ast`, and `kestrel:dev/typecheck/typecheck` mirror the bootstrap compiler's internal categories (`var`, `prim`, `arrow`, `record`, `app`, `tuple`, `union`, `inter`, `scheme`), unification/subtyping entry points (`unify`, `unifySubtype`), AST-to-internal conversion, inferred-type annotation flow, and fresh type-variable id allocation behaviour to keep cross-compiler KTI/typecheck interoperability stable during migration.
...
- **Type-variable id spaces for imports:** Fresh unification variables allocated during the current typecheck run must come from the local fresh-var allocator (`freshVar`) and remain non-negative. Type variables loaded from dependency KTIs must be freshened into a disjoint imported-id space before entering the current environment (TS and self-hosted implementations use negative ids). This prevents accidental unifier aliasing between unrelated dependencies.
...
- **Docs tooling inference surface:** `kestrel doc` reuses the canonical parser/typechecker inference path for exported bindings. For `export val` / `export var` declarations without explicit type annotations, docs signatures are rendered with the inferred type text (same renderer as compiler diagnostics/typecheck tooling). If inference cannot be resolved for a binding, docs emit the stable marker `<inference-unavailable>` instead of failing extraction.
...
- **Await:** The expression `await e` is well-typed only if (1) **e** has type **Task<A>** for some type **A**, and (2) the expression appears in an **async context**. The type of `await e` is **A**. Use of `await` outside an async context (e.g. in a non-async function or at top level) is a **type or context error** (01 §5). The type checker implements this via **unification**: if the type of `e` is an unresolved type variable `α` (as can happen when `e` is a direct recursive call and the callee's type has not yet been fully settled), the type checker constrains `α = Task<A>` for a fresh variable `A` rather than immediately reporting an error. This ensures that `await f(args)` and the equivalent two-step `val t: Task<T> = f(args); await t` are treated identically.
- **Structural async typing:** The `async` keyword is an implementation detail at the type level. A function `async fun f(x: A): Task<B>` has the same type as `fun f(x: A): Task<B>` — namely `(A) -> Task<B>`. Callers cannot distinguish between the two by type alone. Any higher-order function accepting `(A) -> Task<B>` works equally for both. There is no way to require a function to be "truly async" (dispatched on a virtual thread) at the type level; that distinction is an implementation detail invisible to callers.
- **Result-typed async stdlib:** For stdlib operations that encode failure as data, `A` may itself be a `Result<T, E>`. Example: `await Fs.readText(path)` has type `Result<String, FsError>`, `await Fs.listDir(path)` has type `Result<List<DirEntry>, FsError>`, and `await Process.runProcess(program, args)` has type `Result<ProcessResult, ProcessError>`.
- **Task<T>** is a built-in type (01 §3.6, 02); its values are created by calling async functions or by runtime primitives. On the JVM backend these values are represented by `kestrel.runtime.KTask` (wrapping `CompletableFuture<Object>`). The type system does not distinguish "suspended" vs "completed" tasks; both have type Task<T>. For cancellation semantics — `Task.cancel`, the `Cancelled` exception, `Task.all` fail-fast behavior, and `Task.race` edge cases — see `kestrel:task` in [02-stdlib.md](02-stdlib.md) and 01 §5.
