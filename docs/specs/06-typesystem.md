...
- **Result-typed async stdlib:** For stdlib operations that encode failure as data, `A` may itself be a `Result<T, E>`. Example: `await Fs.readText(path)` has type `Result<String, FsError>` and `await Process.runProcess(program, args)` has type `Result<Int, ProcessError>`.
