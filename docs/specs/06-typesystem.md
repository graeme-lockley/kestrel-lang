...
- **Built-in generics:** `Array<T>` and `Task<T>` are part of the runtime; each takes one type argument. `Array<T>` denotes a mutable sequence of values of type `T`; the JVM representation is `kestrel.runtime.KArray`. `Task<T>` denotes an asynchronous computation that yields a value of type `T` (see async/await in the language spec). `Array<T>` operations are exposed through the `kestrel:array` stdlib module: `arrayNew`, `arrayFrom`, `arrayGet`, `arraySet`, `arrayLength`, `arrayPush` (see [02-stdlib.md §kestrel:array](02-stdlib.md)).
