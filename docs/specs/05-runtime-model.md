...
**CHAR operations:** For two **CHAR** values, comparison instructions (**EQ**, **NE**, **LT**, **LE**, **GT**, **GE**) compare their Unicode code points as unsigned scalar values (same ordering as UTF-32 code point order for valid scalars). Formatting and string interpolation must render a CHAR as the UTF-8 encoding of that single scalar (the glyph), not as a decimal integer. Conversions to **STRING** (single code point) and to **INT** (code point as integer) are provided by runtime primitives / stdlib (`kestrel:char`, `kestrel:string`).
...
### 1.0 Type test (`e is T`) at runtime

Well-typed **`e is T`** evaluates to **Bool** per [01-language.md](01-language.md) §3.2.2. The reference VM exposes **KIND_IS** (04 §1.3.1): a single discriminant selects among inline tags (**Int**, **Bool**, **Unit**, **Char**) and heap kinds (**STRING**, **FLOAT**, **RECORD**). **ADT** checks compare **constructor tag** (and payload when required) using the same representation as **MATCH** / **CONSTRUCT** (§2). **Record** shape checks ensure the value is a **RECORD** whose fields exist and satisfy nested **`is`** probes as compiled. Alternative VMs must preserve the **same** boolean results for the same typed inputs as this model and 04.
...
### 1.2 Tail recursion and mutual tail calls (call frames)
...
When resuming the handler, the VM must load **bytecode and constants from the module that contains the handler** (the module that was current when `TRY` executed). That module is not necessarily the same as the **caller** module stored on the call stack for a cross-unit call. If the try block was running **module initializer** code with that module’s **global slots** as the active binding array for that execution context, the handler must continue to use that same array until frame rules change again (e.g. `CALL` into a function body or `RET`).

**Stack traces:** The stdlib (02 kestrel:stack) provides `trace(value)` to obtain a `StackTrace<T>`: a **record** holding **`value`** (the argument, usually the caught exception) and **`frames`** (`List<StackFrame>`). The reference VM builds **`frames`** at the **`__capture_trace`** call site by walking active bytecode frames in **innermost-first** order (the innermost site is the **`CALL`** to the primitive; outer frames use each saved return **PC**), mapping PCs through the debug section (03 §8) when entries exist; frames without mapping still appear with placeholder **file** / **line** (02). The **`function`** field is a placeholder until richer symbols exist. **Programmatic** `format(trace(...))` uses the same per-frame line shape as **uncaught** stderr output (`  at file:line`). On an **uncaught exception**, the VM must print the exception value and a stack trace (one `  at file:line` per frame) to stderr using the debug section where available, then exit with a non-zero status. Implementations may omit stderr lines for frames with no debug mapping; **`trace`** must still yield a **non-empty** `frames` list whenever there is at least one active frame at the capture site.
...
| **02** | Option, Result, List, and stdlib ADTs (including JSON `Value` from `kestrel:json`) are ADT heap objects; Task is TASK heap object; **`StackTrace<T>`** from **kestrel:stack** is a **record** (`value` + **list** of frame records). |
