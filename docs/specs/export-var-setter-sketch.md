...
The above is fully implemented. The setter body emits `LOAD_LOCAL 0; STORE_GLOBAL slot; LOAD_CONST unit; RET` per acceptance criteria. Block-level assignment to an imported var (e.g. inside a block expression or function body) is supported: `importedVarSetterIds` is passed into expression codegen and the block-level `AssignStmt` handler emits `CALL setterId 1` when the target is an imported var. The .kti format matches §5: var entries include `function_index` (getter) and `setter_index` (setter). Round-trip and integration tests validate that all export kinds and type forms are preserved, and that the importer's bytecode includes getter and setter entries in the imported function table (03 §6.6). See compiler test: `compiler/test/unit/types-file.test.ts` (round-trip) and `compiler/test/integration/compile-file.test.ts` (export var + import and assign). Kestrel unit test `tests/unit/export_var.test.ks` (with fixture `tests/fixtures/export_var_helper.ks`) verifies end-to-end: read initial value, assign, read back via getter and via helper function, and multiple assignments.
