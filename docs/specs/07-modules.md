...
| **02** | Standard library module names (including `kestrel:string`, `kestrel:char`, `kestrel:list`, `kestrel:stack`, `kestrel:http`, `kestrel:json`, `kestrel:fs`, `kestrel:web`) must resolve to modules that satisfy 02. No other spec may use those names for a different contract. |
