...
- **Top-level fields:**
  ...
  - `depHashes` (object, v4+): map from absolute path of each direct dependency to the lowercase hex SHA-256 of that dependency's `.kti` file content bytes. Used for transitive invalidation; see §5.2.
...
1. **mtime gate (fast-path, no source read):** Check `stat(.kti).mtime > stat(source.ks).mtime`. If the `.kti` is newer and every entry in `depHashes` matches the SHA-256 of the direct dependency `.kti` content from the in-process cache, the `.kti` is considered fresh without reading the source file. Load the exported environment from the `.kti` and skip parse/typecheck/codegen for this dependency. This is the common case (nothing changed) and requires only 2 `stat()` calls plus one small `.kti` JSON read per dep.
