...
**Tail-call optimization (reference compiler, Zig VM / `.kbc`):** For **top-level** functions in the **same** module, a direct call `g(...)` in **tail position** (same tail sites as below) may be lowered without a **`CALL`** when the callee `g` is another such top-level function (including **self** `f(...)`) and the call’s argument count equals `g`’s arity: arguments are evaluated left-to-right, then **`STORE_LOCAL`** into parameter slots `0 .. arity-1` (last argument popped first so slot `0` receives the first parameter), then **`JUMP`** to the start of `g`’s body (byte offset `0` within `g`’s code chunk in the concatenated module code). Observable semantics match a normal call followed by return. **Not** lowered: calls not in tail position, short-circuit operands (`&`, `|`), **`CALL_INDIRECT`**, nested/block-local `fun`, callees resolved to imports or **`LOAD_IMPORTED_FN`**, namespace-qualified calls, arity mismatch at the call site, or any callee that is not a known sibling top-level function of the current function. **JVM backend:** self-tail in a method is still lowered with **`GOTO`** to the method entry; **cross-function** tail calls use normal **`INVOKESTATIC`** (Java bytecode has no cross-method tail jump), so mutual tail recursion may grow the JVM stack unless the host implements tail-call support separately.
...
- **Lists and ADTs:** CONSTRUCT with list ADT and Nil/Cons, Option, Result, and **any other** exported ADT (including JSON `Value` / `JsonParseError` from `kestrel:json`, which use **normal** per-module `adt_id` rows — not a reserved built-in JSON row). CONSTRUCT_IMPORT for qualified **`M.Ctor(…)`** (07 §2.3); MATCH for pattern matching on constructors and list patterns (01 §3.2, 02).
