...
- [X] Parse `type Opt<T> = None | Some(T)` successfully
- [X] Parse `type Tree<T> = Leaf(T) | Node(Tree<T>, Tree<T>)` (recursive generic)
- [X] Parse `type Result<T, E> = Ok(T) | Err(E)` (multiple type params)
- [X] Type check generic ADT declarations
- [X] Match on generic ADT values works correctly
- [X] Tests: Update `tests/unit/adts.test.ks` to use generic syntax
