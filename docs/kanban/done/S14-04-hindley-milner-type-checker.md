...
- [x] Implement inferred-type annotation storage (`setInferredType`, `getInferredType`) using a Kestrel-friendly replacement for `WeakMap`.
...
- [x] Emit diagnostics using `kestrel:tools/compiler/diagnostics` with stable code strings and messages for unification errors, undefined names, loop-control misuse, and exhaustiveness failures in the implemented MVP surface.
...
- [x] `docs/specs/01-language.md` — reviewed during the port; no language-spec change was required for the implemented MVP surface.

## Build notes

- 2026-04-12: Started implementation.
- 2026-04-12: Shipped an MVP self-hosted checker covering literals, functions, let-polymorphism, top-level declarations, and simple ADT match exhaustiveness with exported type maps.
- 2026-04-12: Replaced the TypeScript `WeakMap` side table with a simple inferred-type entry list keyed by AST structural equality so downstream stories still have `getInferredType` / `setInferredType` helpers.
- 2026-04-12: Broke large helpers such as `checkDecls` and `typecheck` into smaller top-level functions to avoid JVM verifier failures in generated bytecode.
- 2026-04-12: Aligned literal inference with parser output, which tags boolean literals as `true` / `false` and uses `True` / `False` source syntax.
- 2026-04-12: Extended expression coverage in a follow-up pass: added `EUnary`, `ERecord`, `EField` (with namespace/tuple/record dispatch), `EAwait`, `EThrow`, `ETry`, `ECons`, `EPipe` (`|>` and `<|` rewriting), `ETemplate`, `EIs`, and `SAssign`. Each complex form extracted to a dedicated helper to avoid JVM verifier issues. 16 tests now passing.
