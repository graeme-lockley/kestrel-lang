...
- [x] Create `stdlib/kestrel/dev/parser/parser.test.ks` with imports: test harness; `lex` from `kestrel:dev/parser/lexer`; `parse`, `parseExpr`, `ParseError` from `kestrel:dev/parser/parser`; all AST types from `kestrel:dev/parser/ast`
- [x] Add `export fun run(): Unit` entry point
- [x] Implement helper `fun pl(src: String): Program` — calls `parse(lex(src))`, asserts `Ok`, returns program
- [x] Implement helper `fun pe(src: String): Expr` — calls `parseExpr(lex(src))`, asserts `Ok`, returns expr
- [x] Implement helper `fun firstImport(src: String): ImportDecl` — `pl(src).imports.head`
- [x] Implement helper `fun firstDecl(src: String): TopDecl` — `pl(src).body.head`

**Import tests:**
- [x] `import { foo } from "m"` → `IDNamed("m", [{external="foo",local="foo"}])`
- [x] `import { foo as bar } from "m"` → `IDNamed("m", [{external="foo",local="bar"}])`
- [x] `import { a, b } from "m"` → `IDNamed` with 2 ImportSpec entries
- [x] `import * as M from "m"` → `IDNamespace("m","M")`
- [x] `import "m"` → `IDSideEffect("m")`
- [x] Two import statements → `imports` list length is 2

**Function declaration tests:**
- [x] `fun f(x: Int): Int = x` → `TDFun` with `name="f"`, `exported=False`, `async_=False`, params length 1, param name `"x"`
- [x] `async fun f(): Task<Int> = 1` → `TDFun { async_=True }`
- [x] `export fun f(): Unit = ()` → `TDFun { exported=True }`
- [x] `fun f<A>(x: A): A = x` → `TDFun { typeParams=["A"] }`
- [x] `fun f(x: Int, y: Bool): String = "z"` → `TDFun` with 2 params

**Value and variable declaration tests:**
- [x] `val x = 42` → `TDSVal("x", ELit("int","42"))` (no type annotation)
- [x] `var x = 42` → `TDSVar("x", ELit("int","42"))`
- [x] `export val x: Int = 42` → `TDVal("x", Some(ATPrim("Int")), ELit("int","42"))`
- [x] `export var x: Int = 0` → `TDVar("x", Some(ATPrim("Int")), ELit("int","0"))`

**Type declaration tests:**
- [x] `type Alias = Int` → `TDType` with `visibility="local"`, `body=TBAlias(ATPrim("Int"))`
- [x] `type Color = Red | Green | Blue` → `TDType { body=TBAdt(...) }` with 3 constructors
- [x] `type Box<A> = Box(A)` → `TDType { typeParams=["A"] }`
- [x] `opaque type T = Int` → `TDType { visibility="opaque" }`
- [x] `exception MyError` → `TDException { exported=False, name="MyError", fields=None }`
- [x] `exception E { msg: String }` → `TDException { fields=Some([{name="msg",...}]) }`
- [x] `export exception E` → `TDException { exported=True }`
- [x] `export * from "m"` → `TDExport(EIStar("m"))`
- [x] `export { foo, bar } from "m"` → `TDExport(EINamed("m", ...))`

**Literal expression tests:**
- [x] `pe "42"` equals `ELit("int","42")`
- [x] `pe "0xff"` equals `ELit("int","0xff")`
- [x] `pe "3.14"` equals `ELit("float","3.14")`
- [x] `pe "\"hello\""` equals `ELit("string","hello")` (decoded, without quotes)
- [x] `pe "'a'"` equals `ELit("char","a")` (decoded)
- [x] `pe "()"` equals `ELit("unit","()")`
- [x] `pe "True"` equals `ELit("true","True")`
- [x] `pe "False"` equals `ELit("false","False")`

**Identifier, field access, and call tests:**
- [x] `pe "x"` equals `EIdent("x")`
- [x] `pe "Foo"` equals `EIdent("Foo")`
- [x] `pe "f()"` equals `ECall(EIdent("f"), [])`
- [x] `pe "f(1)"` equals `ECall(EIdent("f"), [ELit("int","1")])`
- [x] `pe "f(1, 2)"` — args list has length 2
- [x] `pe "a.b"` equals `EField(EIdent("a"), "b")`
- [x] `pe "a.b.c"` equals `EField(EField(EIdent("a"),"b"), "c")`
- [x] `pe "a.0"` equals `EField(EIdent("a"), "0")` (tuple index)
- [x] `pe "f(1)(2)"` — outer is `ECall` of `ECall` (chained call)

**Operator precedence tests:**
- [x] `pe "1 + 2"` → `EBinary("+", ELit("int","1"), ELit("int","2"))`
- [x] `pe "1 + 2 * 3"` → `EBinary("+", 1, EBinary("*", 2, 3))` — mul before add
- [x] `pe "1 * 2 + 3"` → `EBinary("+", EBinary("*",1,2), 3)` — mul before add, left side
- [x] `pe "2 ** 3 ** 4"` → `EBinary("**", 2, EBinary("**",3,4))` — right-associative
- [x] `pe "1 - 2 - 3"` → `EBinary("-", EBinary("-",1,2), 3)` — left-associative
- [x] `pe "1 == 2"` → `EBinary("==", ELit, ELit)`
- [x] `pe "1 != 2"` → `EBinary("!=", ...)`
- [x] `pe "a < b"` → `EBinary("<", EIdent"a", EIdent"b")`
- [x] `pe "a >= b"` → `EBinary(">=", ...)`
- [x] `pe "a | b"` → `EBinary("|", EIdent"a", EIdent"b")`
- [x] `pe "a & b"` → `EBinary("&", EIdent"a", EIdent"b")`
- [x] `pe "x |> f"` → `EPipe("|>", EIdent"x", EIdent"f")`
- [x] `pe "f <| x"` → `EPipe("<|", EIdent"f", EIdent"x")`
- [x] `pe "x :: xs"` → `ECons(EIdent"x", EIdent"xs")`
- [x] `pe "1 :: 2 :: []"` → `ECons(ELit, ECons(ELit, EList([])))` — right-associative
- [x] `pe "-1"` → `EUnary("-", ELit("int","1"))`
- [x] `pe "!x"` → `EUnary("!", EIdent"x")`
- [x] `pe "x is Int"` → `EIs(EIdent"x", ATPrim("Int"))`

**Control flow tests:**
- [x] `pe "if (x) y else z"` → `EIf(EIdent"x", EIdent"y", Some(EIdent"z"))`
- [x] `pe "if (x) y"` → `EIf(EIdent"x", EIdent"y", None)`
- [x] `pe "while (x) { y }"` → `EWhile(EIdent"x", Block{stmts=[], result=EIdent"y"})`
- [x] `pe "await x"` → `EAwait(EIdent"x")`
- [x] `pe "throw e"` → `EThrow(EIdent"e")`
- [x] `pe "try { x } catch { E => 0 }"` → `ETry(Block{...}, None, [Case_{...}])`

**Lambda tests:**
- [x] `pe "(x: Int) => x"` → `ELambda(False, [], [{name="x",type_=Some(ATPrim"Int")}], EIdent"x")`
- [x] `pe "(x) => x"` → `ELambda(False, [], [{name="x",type_=None}], EIdent"x")`
- [x] `pe "(x, y) => x"` → `ELambda` with 2 params
- [x] `pe "async (x) => x"` → `ELambda(True, [], [{name="x",...}], EIdent"x")`
- [x] Backtracking test — `pe "(x)"` should be `EIdent "x"` (grouping, not lambda — no `=>`)

**Match tests:**
- [x] `pe "match (x) { 1 => 2 }"` → `EMatch(EIdent"x", [Case_{pattern=PLit("int","1"), body=ELit("int","2")}])`
- [x] Match with multiple cases — two `=>` clauses → cases list length 2
- [x] Match `_` arm → `Case_{ pattern=PWild }`
- [x] Match variable arm `x` → `Case_{ pattern=PVar("x") }`
- [x] Match constructor `Some(v)` → `Case_{ pattern=PCon("Some",[...]) }`

**Collection and record tests:**
- [x] `pe "[1, 2, 3]"` → `EList([LElem(ELit), LElem(ELit), LElem(ELit)])`
- [x] `pe "[...xs, 1]"` → `EList([LSpread(EIdent"xs"), LElem(ELit)])`
- [x] `pe "[]"` → `EList([])`
- [x] `pe "{x = 1, y = 2}"` → `ERecord(None, [field"x", field"y"])`
- [x] `pe "{mut x = 1}"` → `ERecord(None, [{name="x",mut_=True,...}])`
- [x] `pe "{...r, x = 1}"` → `ERecord(Some(EIdent"r"), [...])`
- [x] `pe "{}"` → `ERecord(None, [])`
- [x] `pe "(1, 2)"` → `ETuple([ELit("int","1"), ELit("int","2")])`
- [x] `pe "(1, 2, 3)"` → `ETuple` with 3 elements
- [x] `pe "(x)"` → `EIdent "x"` (grouping, not tuple)

**Block tests:**
- [x] `pe "{x}"` → `EBlock({stmts=[], result=EIdent"x"})`
- [x] `pe "{val x = 1; x}"` → `EBlock({stmts=[SVal("x",None,ELit("int","1"))], result=EIdent"x"})`
- [x] `pe "{var x = 1; x := 2; x}"` → `EBlock` with `SVar`, `SAssign`, and `result=EIdent"x"`
- [x] `pe "{1; 2}"` → `EBlock({stmts=[SExpr(ELit("int","1"))], result=ELit("int","2")})`
- [x] `pe "{break}"` → `EBlock({stmts=[SBreak], result=ENever})`

**Template/string tests:**
- [x] `pe "\"${x}\""` → `ETemplate([TmplExpr(EIdent"x")])`
- [x] `pe "\"hello ${name}\""` → `ETemplate([TmplLit("hello "), TmplExpr(EIdent"name")])`
- [x] `pe "\"$name\""` → `ETemplate([TmplExpr(EIdent"name")])`
- [x] `pe "\"a ${1 + 2} b\""` → `ETemplate([TmplLit"a ", TmplExpr(EBinary("+",...)), TmplLit" b"])`

**Type annotation tests (parsed inside declarations):**
- [x] `fun f(x: Int): Bool = True` — param type is `ATPrim("Int")`, return type is `ATPrim("Bool")`
- [x] `fun f(x: List<Int>): Unit = ()` — param type is `ATApp("List", [ATPrim("Int")])`
- [x] `fun f(x: Int -> Bool): Unit = ()` — param type is `ATArrow([ATPrim("Int")], ATPrim("Bool"))`
- [x] `fun f(x: {a: Int}): Unit = ()` — param type is `ATRecord([{name="a",...}])`
- [x] `fun f(x: A | B): Unit = ()` — param type is `ATUnion(ATIdent"A", ATIdent"B")`
- [x] `fun f(x: A * B): Unit = ()` — param type is `ATTuple([ATIdent"A", ATIdent"B"])`

**Pattern tests (inside match expressions):**
- [x] Wildcard `_` → `PWild`
- [x] Variable `x` → `PVar("x")`
- [x] Int literal `42` → `PLit("int","42")`
- [x] Float literal `3.14` → `PLit("float","3.14")`
- [x] String literal `"s"` → `PLit("string","s")`
- [x] `True` → `PCon("True",[])`; `False` → `PCon("False",[])`
- [x] Constructor no-arg `None` → `PCon("None",[])`
- [x] Constructor positional `Some(x)` → `PCon("Some",[{name="__field_0",pattern=Some(PVar"x")}])`
- [x] Constructor record `Con{x=p}` → `PCon("Con",[{name="x",pattern=Some(PVar"p")}])`
- [x] Constructor record shorthand `Con{x}` → `PCon("Con",[{name="x",pattern=Some(PVar"x")}])`
- [x] List pattern `[a, b]` → `PList([PVar"a",PVar"b"],None)`
- [x] List with rest `[a, ...xs]` → `PList([PVar"a"],Some("xs"))`
- [x] Empty list `[]` → `PList([],None)`
- [x] Cons `h :: t` → `PCons(PVar"h",PVar"t")`
- [x] Tuple `(a,b)` → `PTuple([PVar"a",PVar"b"])`
- [x] Unit `()` → `PLit("unit","()")`

**Final verification:**
- [x] Run: `./kestrel test stdlib/kestrel/dev/parser/parser.test.ks` → confirm compile error (`parser.ks` not yet written — **red phase**)

### Phase H — Parser — *TDD green* (`stdlib/kestrel/dev/parser/parser.ks`)

- [x] Create `stdlib/kestrel/dev/parser/parser.ks` with all necessary imports (token types, all AST types, `lex` from lexer)

**ParseError and ParseState:**
- [x] Define `export type ParseError = ParseError(String, Int, Int, Int)` — (message, offset, line, col)
- [x] Define `ParseState` mutable record:
  ```
  type ParseState = {
...
- [x] Implement `fun buildFiltered(toks: List<Token.Token>, arr: Array<Token.Token>): Unit` — append each non-trivia token to `arr` (match on kind, skip `Token.TkWs`, `Token.TkLineComment`, `Token.TkBlockComment`)
- [x] Implement `fun makePs(allTokens: List<Token.Token>): ParseState` — create array, call `buildFiltered`, return `{ tokens=arr, count=Arr.length(arr), mut pos=0, mut errors=[] }`
- [x] Implement `fun psCurrent(ps: ParseState): Token.Token` — `Arr.get(ps.tokens, min(ps.pos, ps.count - 1))`
- [x] Implement `fun psPeek(ps: ParseState, offset: Int): Token.Token` — `Arr.get(ps.tokens, min(ps.pos + offset, ps.count - 1))`
- [x] Implement `fun psAdvance(ps: ParseState): Token.Token` — return `psCurrent`; if not EOF increment `ps.pos`
- [x] Implement `fun psAtKw(ps: ParseState, kw: String): Bool`
- [x] Implement `fun psAtOp(ps: ParseState, op: String): Bool`
- [x] Implement `fun psAtPunct(ps: ParseState, ch: String): Bool`
- [x] Implement `fun psAtIdent(ps: ParseState): Bool`
- [x] Implement `fun psAtUpper(ps: ParseState): Bool`
- [x] Implement `fun psAtEof(ps: ParseState): Bool`
- [x] Implement `fun psError(ps: ParseState, msg: String): ParseError` — construct `ParseError(msg, span.start, span.line, span.col)` from current token's span; prepend to `ps.errors`; return the error
- [x] Implement `fun psExpectIdent(ps: ParseState): Result<String, ParseError>` — if `Token.TkIdent` advance and return `Ok(text)`, else `Err(psError(...))`
- [x] Implement `fun psExpectUpper(ps: ParseState): Result<String, ParseError>`
- [x] Implement `fun psExpectKw(ps: ParseState, kw: String): Result<Token.Token, ParseError>`
- [x] Implement `fun psExpectOp(ps: ParseState, op: String): Result<Token.Token, ParseError>`
- [x] Implement `fun psExpectPunct(ps: ParseState, ch: String): Result<Token.Token, ParseError>`
- [x] Implement `fun psExpectStr(ps: ParseState): Result<String, ParseError>` — if `Token.TkStr` advance and return `Ok(rawText)`, else error
- [x] Implement `fun isExprStart(ps: ParseState): Bool` — returns True if current token can start an expression: `Token.TkInt`, `Token.TkFloat`, `Token.TkStr`, `Token.TkTemplate`, `Token.TkChar`, `Token.TkIdent`, `Token.TkUpper`, or kw/punct `( [ { if while match try throw -` `+ ! await async`

**Entry points:**
- [x] Implement `export fun parse(tokens: List<Token.Token>): Result<Program, ParseError>` — `makePs`, call `parseProgram`, return `Err(head errors)` if errors non-empty, else `Ok(prog)`
- [x] Implement `export fun parseExpr(tokens: List<Token.Token>): Result<Expr, ParseError>` — `makePs`, call `parsePipeExpr(ps, "expr")`, return as above

**Program-level parsing:**
- [x] Implement `fun parseProgram(ps: ParseState): Program` — call `parseImports`, then `parseTopBody`, return `{ imports, body }`
- [x] Implement `fun parseImports(ps: ParseState): List<ImportDecl>` — loop while `psAtKw(ps, "import")`: advance past `import`, call `parseImport1`, collect results
- [x] Implement `fun parseImport1(ps: ParseState): ImportDecl` — three forms:
...
- [x] Implement `fun parseImportSpecList(ps: ParseState): List<ImportSpec>` — comma-separated `name` or `name as alias`; consume `}`
- [x] Implement `fun parseTopBody(ps: ParseState): List<TopDecl>` — loop until EOF; dispatch:
  - `export` → `parseExport`
...
- [x] Implement `fun parseExport(ps: ParseState): TopDecl` — consume `export`, then:
...
- [x] Implement `fun parseTopDecl(ps: ParseState, exported: Bool): TopDecl` — dispatch on keyword:
  ...
  - `var` → same → `TDVar`
- [x] Implement `fun parseTopStmt(ps: ParseState): TopDecl` — if `val`: consume, ident, `=`, expr → `TDSVal` (NO type); if `var`: same → `TDSVar`; else parse expr; if `:=` follows → consume, parse rhs → `TDSAssign`; else → `TDSExpr`

**Declaration helpers:**
- [x] Implement `fun parseTypeParams(ps: ParseState): List<String>` — if `<` (op), consume, comma-separated Token.TkIdent names, consume `>`; else return `[]`
- [x] Implement `fun parseSingleParam(ps: ParseState): Param` — `psExpectIdent`; if `:` (punct) follows, consume and parse type → `{ name, type_=Some(t) }`; else `{ name, type_=None }`
- [x] Implement `fun parseParamList(ps: ParseState): List<Param>` — consume `(`; comma-separated `parseSingleParam`; consume `)`
- [x] Implement `fun parseFunDecl(ps: ParseState, exported: Bool): TopDecl` — consume optional `async` (record flag), consume `fun`, `psExpectIdent` (name), `parseTypeParams`, `parseParamList`, consume `:`, `parseTypeExpr` (retType), consume `=`, `parsePipeExpr` (body) → `TDFun(FunDecl{exported, async_, name, typeParams, params, retType, body})`
- [x] Implement `fun parseSingleCtor(ps: ParseState): CtorDef` — `psExpectUpper` (name); if `(` optionally parse comma-separated types until `)` → `{ name, params=types }`; else `{ name, params=[] }`
- [x] Implement `fun parseCtorList(ps: ParseState): List<CtorDef>` — parse `parseSingleCtor`; while `|` (op) follows consume and parse another; return list
- [x] Implement `fun parseTypeDecl(ps: ParseState, exported: Bool): TopDecl` — consume `opaque`? then `type`; name; `parseTypeParams`; consume `=`; decide body: peek — if `Token.TkUpper` and next peek is `(` or `|` → `TBAdt(parseCtorList(ps))`; else `TBAlias(parseTypeExpr(ps))` → `TDType(TypeDecl{visibility, name, typeParams, body})`
- [x] Implement `fun parseExceptionDecl(ps: ParseState, exported: Bool): TopDecl` — consume `exception`; name; if `{` follows parse comma-separated `mut? name: T` fields until `}`; return `TDException(ExceptionDecl{exported, name, fields})`
- [x] Implement `fun parseExternFun(ps: ParseState, exported: Bool): TopDecl` — consume `extern fun`; name; typeParams; paramList; `:` retType; `=` `jvm("desc")` → `TDExternFun`
- [x] Implement `fun parseExternType(ps: ParseState): TopDecl` — handle `extern opaque export type` / `extern opaque type` / `extern type`; name; typeParams; `=` `jvm("class")` → `TDExternType`
- [x] Implement `fun parseExternImport(ps: ParseState): TopDecl` — consume `extern import`; target str; `as`; alias ident; optional `{ name(params): retType, ... }` overrides → `TDExternImport`

**Type expression parsing:**
- [x] Implement `fun parseTypeExpr(ps: ParseState): AstType` — entry point; call `parseOrType`
- [x] Implement `fun parseOrType(ps: ParseState): AstType` — parse `parseAndType`; while `|` (op, but NOT `||`) left-associate into `ATUnion`
- [x] Implement `fun parseAndType(ps: ParseState): AstType` — parse `parseArrowType`; while `&` left-associate into `ATInter`
- [x] Implement `fun parseArrowType(ps: ParseState): AstType` — parse `parseAppType`; if `->` (op) right-associate into `ATArrow([lhs], rhs)` (if lhs was a tuple product, use its list as param types)
- [x] Implement `fun parseAppType(ps: ParseState): AstType` — parse `parseAtomType`; if `<` try consuming generic args list → `ATApp(name, args)`; while `*` (op) left-associate into `ATTuple`
- [x] Implement `fun parseAtomType(ps: ParseState): AstType` — dispatch: `(` `)` → `ATPrim "Unit"`; `(T)` → `T` (grouping); `{ }` → `ATRecord([])`; `{ ...r }` → `ATRowVar(r)`; `{ fields }` → `ATRecord(parseRecordTypeFields)`; `Token.TkIdent`/`Token.TkUpper` → `ATPrim` or `ATIdent`; `Token.TkIdent` `.` `Token.TkUpper` (qualified) → `ATQualified`
- [x] Implement `fun parseRecordTypeFields(ps: ParseState): List<TypeField>` — comma-separated `mut? name: T` fields until `}`; consume `}`

**Expression parsing — precedence levels:**
- [x] Implement `fun parsePipeExpr(ps: ParseState, ctx: String): Expr` — call `parseConsExpr`; while `|>` or `<|` (op) left-associate into `EPipe(op, l, r)`
- [x] Implement `fun parseConsExpr(ps: ParseState, ctx: String): Expr` — call `parseOrExpr`; if `::` right-associate into `ECons(lhs, parseConsExpr(ps, ctx))`
- [x] Implement `fun parseOrExpr(ps: ParseState, ctx: String): Expr` — call `parseAndExpr`; while `|` (single `|`, NOT `|>`) left-associate into `EBinary("|", l, r)`
- [x] Implement `fun parseAndExpr(ps: ParseState, ctx: String): Expr` — call `parseIsExpr`; while `&` left-associate into `EBinary("&", l, r)`
- [x] Implement `fun parseIsExpr(ps: ParseState, ctx: String): Expr` — call `parseRelExpr`; if `is` kw consume and parse type → `EIs(expr, t)` (no chaining)
- [x] Implement `fun parseRelExpr(ps: ParseState, ctx: String): Expr` — call `parseAddExpr`; while op is `==`,`!=`,`<`,`>`,`<=`,`>=` left-associate → `EBinary`
- [x] Implement `fun parseAddExpr(ps: ParseState, ctx: String): Expr` — call `parseMulExpr`; while `+` or `-` left-associate → `EBinary`
- [x] Implement `fun parseMulExpr(ps: ParseState, ctx: String): Expr` — call `parsePowExpr`; while `*`,`/`,`%` left-associate → `EBinary`
- [x] Implement `fun parsePowExpr(ps: ParseState, ctx: String): Expr` — call `parseUnary`; if `**` right-associate `EBinary("**", lhs, parsePowExpr(ps, ctx))`
- [x] Implement `fun parseUnary(ps: ParseState, ctx: String): Expr` — if op is `-`,`+`,`!` consume and wrap `EUnary(op, parseUnary)`; if `async` and next is `(` → `tryParseParenLambda(ps, True)`; if `<` try `isGenericLambdaHead` → `parseGenericLambda`; else delegate to `parsePrimary`
- [x] Implement `fun exprMayBeCallCallee(e: Expr): Bool` — returns `True` for `EIdent`, `EField`, `ECall`, `ELambda`, `EAwait(inner)` where `exprMayBeCallCallee(inner)` is also True; returns `False` for all literal/collection/block forms
- [x] Implement `fun parsePrimary(ps: ParseState, ctx: String): Expr` — check for `await` prefix (consume if present); call `parseAtom`; postfix loop:
...
- [x] Implement `fun parseArgList(ps: ParseState): List<Expr>` — comma-separated `parsePipeExpr` until `)`; do NOT consume `)`
- [x] Implement `fun parseAtom(ps: ParseState, ctx: String): Expr` — dispatch on current token:
...
- [x] Implement `fun decodeString(raw: String): String` — strip surrounding `"` delimiters (`Str.slice(raw, 1, Str.length(raw) - 1)`); process escape sequences: scan char by char; on `\` decode escape; accumulate to result string
- [x] Implement `fun decodeChar(raw: String): String` — strip `'` delimiters; decode single escape or return the single character

**Template parsing:**
- [x] Implement `fun parseTemplateExpr(ps: ParseState, parts: List<Token.TemplatePart>): Expr` — map over parts: `Token.TPLiteral(s)` → `TmplLit(s)`; `Token.TPInterp(src)` → call `lex(src)` then `parseExpr` from this file, extract expr → `TmplExpr(expr)`; return `ETemplate(mappedParts)`

**Lambda backtracking:**
- [x] Implement `fun tryParseParamListOnly(ps: ParseState): Option<List<Param>>` — speculative parse: save `ps.pos` and `ps.errors`; attempt to parse comma-separated params where each is `Token.TkIdent` optionally followed by `: type`; if any element fails or unexpected token found, restore `ps.pos := savedPos` and `ps.errors := savedErrors`, return `None`; on success return `Some(params)`
- [x] Implement `fun tryParseParenLambda(ps: ParseState, async_: Bool): Option<Expr>` — save `ps.pos` and `ps.errors`; advance past `(`; if immediately `)` restore and return `None`; call `tryParseParamListOnly`; on `None` restore and return `None`; if not `)` restore and return `None`; advance past `)`; if not `=>` restore and return `None`; advance past `=>`; parse body with `parsePipeExpr`; return `Some(ELambda(async_, [], params, body))`
- [x] Implement `fun isGenericLambdaHead(ps: ParseState): Bool` — save pos; try consuming `<`, comma-separated Token.TkIdent, `>`; check for `(`; restore pos; return whether all steps matched
- [x] Implement `fun parseGenericLambda(ps: ParseState, async_: Bool): Expr` — consume `<`, typeParams, `>`, `(`, paramList, `)`, `=>`, body; return `ELambda(async_, typeParams, params, body)`

**Block parsing:**
- [x] Implement `fun parseRecordOrBlock(ps: ParseState, ctx: String): Expr` — look at token after `{` (using `psPeek(ps, 1)`):
  ...
  - else → block
- [x] Implement `fun parseRecordExpr(ps: ParseState): Expr` — consume `{`; optional `...expr ,`; comma-separated `mut? name = expr` fields; consume `}`; return `ERecord(spread, fields)`
- [x] Implement `fun parseBlock(ps: ParseState, ctx: String): Block` — consume `{`; accumulate stmts:
...
- [x] Implement `fun parseIfExpr(ps: ParseState, ctx: String): Expr` — consume `if`, `(`, condition, `)`; parse then-expr; if `else` kw follows consume and parse else-expr; return `EIf(cond, then, maybeElse)`
- [x] Implement `fun parseWhileExpr(ps: ParseState): Expr` — consume `while`, `(`, condition, `)`; parse block; return `EWhile(cond, block)`
- [x] Implement `fun parseMatchExpr(ps: ParseState, ctx: String): Expr` — consume `match`, `(`, scrutinee, `)`; `{`; comma-separated `pattern => expr` cases; consume `}`; return `EMatch(scrutinee, cases)`
- [x] Implement `fun parseTryExpr(ps: ParseState, ctx: String): Expr` — consume `try`; parse block body; consume `catch`; optional `(` `var`? ident `)` for catch variable; `{`; comma-separated cases; consume `}`; return `ETry(body, optVar, cases)`
- [x] Implement `fun parseListLiteral(ps: ParseState, ctx: String): Expr` — consume `[`; comma-separated: `...expr` → `LSpread`; else → `LElem`; consume `]`; return `EList`

**Pattern parsing:**
- [x] Implement `fun parsePattern(ps: ParseState): Pattern` — call `parsePatternPrimary`; if `::` (op) follows consume and recurse → `PCons(lhs, parsePattern(ps))`
- [x] Implement `fun parseConPatternFields(ps: ParseState): List<ConField>` — consume `(`; comma-separated patterns, naming them `"__field_0"`, `"__field_1"`, …; consume `)`; return list
- [x] Implement `fun parseConRecordFields(ps: ParseState): List<ConField>` — consume `{`; comma-separated: `name = pattern` → `{name, pattern=Some(p)}`; bare `name` → `{name, pattern=Some(PVar(name))}` (shorthand binding); consume `}`; return list
- [x] Implement `fun parseListPattern(ps: ParseState): Pattern` — consume `[`; comma-separated patterns; optional `...rest` at end (Token.TkIdent after `...` op); consume `]`; return `PList(patterns, optRest)`
- [x] Implement `fun parsePatternPrimary(ps: ParseState): Pattern` — dispatch:
...
- [x] Verify file compiles: `./kestrel build stdlib/kestrel/dev/parser/parser.ks`
- [x] Run: `./kestrel test stdlib/kestrel/dev/parser/parser.test.ks` → confirm all parser tests pass (**green**)
- [x] Run all stdlib tests: `./kestrel test --summary`
- [x] Run compiler tests: `cd compiler && npm test`
...
- [x] `docs/specs/02-stdlib.md` — add `kestrel:dev/parser/token`, `kestrel:dev/parser/ast`, `kestrel:dev/parser/lexer`, and `kestrel:dev/parser/parser` module entries documenting their public APIs.
...
- 2026-04-06: Phases E+F complete. lexer.ks and lexer.test.ks created; 123/123 tests green.
- 2026-04-07: Phases G+H complete. parser.test.ks and parser.ks created; 142/142 tests green. Three parser bugs fixed: (1) `True`/`False` were checked with `atKw()` but are `TkUpper` not `TkKw`; (2) `_` wildcard checked with `atKw("_")` but `_` is `TkIdent`; (3) `a.0` tuple index — `.0` is lexed as `TkFloat ".0"`, handled in parsePrimary by detecting TkFloat tokens starting with `.`. Also required fixes to two JVM codegen bugs: `varSlots` not saved/restored at BlockExpr exit (caused ClassCastException in sibling branches), and `nextLocal` not synced after `recordSlot` allocation (caused silent VerifyError/exit-0 when test suite ran). Plus async failure propagation fix in KRuntime so JVM VerifyErrors surface as non-zero exit.
- 2026-04-07: docs/specs/02-stdlib.md updated with public API entries for all four sub-modules. All 1459 Kestrel tests and 420 compiler tests pass. Story complete.
