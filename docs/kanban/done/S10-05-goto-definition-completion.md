...
- [x] Add `vscode-kestrel/src/server/providers/definition.ts` that resolves same-file `IdentExpr`/`IdentType` names to declaration locations in `Program.body`.
- [x] Add `vscode-kestrel/src/server/providers/completion.ts` that returns keyword, import-name, top-level declaration, and constructor completions.
- [x] Register `onDefinition` and `onCompletion` handlers in `vscode-kestrel/src/server/server.ts` and advertise capabilities.
- [x] Add `vscode-kestrel/test/unit/definition.test.ts` for resolved and unresolved name lookup.
- [x] Add `vscode-kestrel/test/unit/completion.test.ts` for keyword/import/declaration completion coverage.
...
- [x] `docs/specs/01-language.md` — verified import/declaration forms consumed by completion and definition providers are aligned; no textual spec change required in this story.
- [x] `docs/specs/09-tools.md` — no change in this story; consolidated editor capability docs remain in S10-09.

## Build notes

- 2026-04-12: Started implementation.
- 2026-04-12: Added same-file definition provider and completion provider with keyword/import/declaration/constructor items.
- 2026-04-12: Registered definition and completion LSP capabilities/handlers in the language server.
- 2026-04-12: Added definition/completion unit tests; `cd vscode-kestrel && npm run compile && npm test` PASS.
- 2026-04-12: Verification results: `cd compiler && npm run build && npm test` PASS.
- 2026-04-12: Repository-wide `./scripts/kestrel test` currently fails on unrelated in-progress compiler work at `stdlib/kestrel/tools/compiler/codegen.ks` (`Unknown variable: bytes`), outside S10-05 scope.
