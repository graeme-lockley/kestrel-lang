...
- [x] Add `vscode-kestrel/src/server/providers/symbols.ts` with declaration collection from `Program.body` and ADT constructor children under `TypeDecl`.
- [x] Add `vscode-kestrel/src/server/providers/folding.ts` for AST-based fold ranges and `/* ... */` multi-line comment fold ranges.
- [x] Wire `documentSymbolProvider` and `foldingRangeProvider` into `vscode-kestrel/src/server/server.ts` and add handlers using cached `DocumentManager` state.
- [x] Add unit tests in `vscode-kestrel/test/unit/symbols.test.ts` for top-level declaration and constructor child symbols.
- [x] Add unit tests in `vscode-kestrel/test/unit/folding.test.ts` for function/type block and multiline comment folding ranges.
...
- [x] `docs/specs/01-language.md` — verified declaration forms consumed by symbol provider are aligned with spec; no textual spec change required in this story.
- [x] `docs/specs/09-tools.md` — no change in this story; protocol capability listing is documented in S10-09.

## Build notes

- 2026-04-12: Started implementation.
- 2026-04-12: Added `symbols.ts` and `folding.ts` providers and registered document symbol/folding capabilities in the language server.
- 2026-04-12: Added symbols/folding unit tests and verified `vscode-kestrel` compile+tests pass.
- 2026-04-12: Verification results: `cd compiler && npm run build && npm test` PASS; `./scripts/kestrel test` PASS (1759 passed).
