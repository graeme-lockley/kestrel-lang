...
| Area | Change |
|------|--------|
| Stdlib | Add `stdlib/kestrel/compiler/opcodes.ks` with ~80 JVM opcode `val` constants, 18 access-flag constants, and three helpers: `descriptorForType`, `methodDescriptor`, `jvmSlotSize`. |
| Stdlib deps | Imports `kestrel:compiler/types` for `InternalType` (S14-02). |
| Kestrel tests | Add `stdlib/kestrel/compiler/opcodes.test.ks` verifying selected opcode values, descriptor generation, and slot sizes. |
| Docs | Update `docs/guide.md` with `kestrel:compiler/opcodes` module table row. |

## Tasks

- [x] Create `stdlib/kestrel/compiler/opcodes.ks` with all ~80 JVM opcode `val` constants mirroring `compiler/src/jvm-codegen/opcodes.ts`.
...
- [x] Add `stdlib/kestrel/compiler/opcodes.test.ks` with tests covering: `nop=0`, `areturn=176`, `invokevirtual=182`, `accPublic=1`, `descriptorForType(tInt)="J"`, `descriptorForType(tBool)="Ljava/lang/Boolean;"`, `methodDescriptor([tInt], tString)`, and `jvmSlotSize(tInt)=2`.
- [x] Run `cd compiler && npm test`.
- [x] Run `./kestrel test stdlib/kestrel/compiler/opcodes.test.ks`.

## Tests to add

| Layer | Path | Intent |
|-------|------|--------|
| Kestrel harness | `stdlib/kestrel/compiler/opcodes.test.ks` | Verify key opcode constants have correct byte values (NOP=0, ARETURN=0xB0, INVOKEVIRTUAL=0xB6). |
| Kestrel harness | `stdlib/kestrel/compiler/opcodes.test.ks` | Verify `descriptorForType` for primitives: Int→J, Bool→Ljava/lang/Boolean;, String→Ljava/lang/String;. |
| Kestrel harness | `stdlib/kestrel/compiler/opcodes.test.ks` | Verify `methodDescriptor` builds correct `(...)R` string. |
| Kestrel harness | `stdlib/kestrel/compiler/opcodes.test.ks` | Verify `jvmSlotSize` returns 2 for Int/Float, 1 for Bool/String. |

## Documentation and specs to update

- [x] `docs/guide.md` — add `kestrel:compiler/opcodes` row to the compiler dev module table.
