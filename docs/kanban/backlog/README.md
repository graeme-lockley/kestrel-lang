...
The canonical priority order remains the **`NN`** prefix in filenames under `unplanned/` (and the same name as files move through `planned/`, `doing/`, `done/`). Pre-roadmap investigations and ideas belong in **`../future/`** (`slug.md`, no numeric prefix).
