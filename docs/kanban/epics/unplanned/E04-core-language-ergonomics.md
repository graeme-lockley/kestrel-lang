# Epic E04: Core Language Ergonomics

## Status

Unplanned

## Summary

Improves day-to-day language ergonomics with explicit discard semantics and a built-in mutable array type.

## Stories

- [S04-01-array-builtin-type.md](../../unplanned/S04-01-array-builtin-type.md)
- [S04-02-ignore-keyword-for-discarding-values.md](../../unplanned/S04-02-ignore-keyword-for-discarding-values.md)

## Dependencies

- None required between member stories.

## Epic Completion Criteria

- Array runtime/typecheck/codegen behavior is implemented with tests.
- `ignore` syntax and diagnostics are implemented with tests.
- Related language/spec updates are merged.
