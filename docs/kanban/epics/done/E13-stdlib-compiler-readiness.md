...
1. [S13-01 — Binary file I/O (`readBytes`/`writeBytes`/`appendBytes`)](../../done/S13-01-binary-file-io.md)
   Adds `ByteArray` opaque type and `readBytes`, `writeBytes`, `appendBytes` to `kestrel:io/fs`. Required for JVM `.class` file emission.

2. [S13-02 — `mkdirAll` and `stat` / file metadata](../../done/S13-02-mkdir-stat.md)
...
3. [S13-03 — Process exit code (`exit`)](../../done/S13-03-process-exit.md)
...
4. [S13-04 — Stderr output (`printErr` / `eprintln`)](../../done/S13-04-stderr-output.md)
...
5. [S13-05 — Cryptographic hashing (`sha256`, `sha1`, `md5`)](../../done/S13-05-crypto-hash.md)
...
6. [S13-06 — Path utilities (`kestrel:sys/path`)](../../done/S13-06-sys-path.md)
...
7. [S13-07 — `Float` parsing (`parseFloat`, `toFloat`)](../../done/S13-07-float-parsing.md)
...
8. [S13-08 — Generic `List.sortBy` and `List.sortWith`](../../done/S13-08-list-sort.md)
...
9. [S13-09 — `List.find`, `List.findIndex`, `List.findMap`, `List.last`](../../done/S13-09-list-find.md)
...
10. [S13-10 — `String` additions: `parseFloat`, base-N int parse, `format` / left-pad-int, `indexOfChar`](../../done/S13-10-string-additions.md)
...
11. [S13-11 — Recursive `listDir` and `collectFiles`](../../done/S13-11-recursive-listdir.md)
...
12. [S13-12 — `Dict` with structural-equality keys (`StructDict`)](../../done/S13-12-struct-dict.md)
...
13. [S13-13 — `kestrel:data/string` `parseIntBase`, `toHexString`, `toBinaryString`](../../done/S13-13-string-numeric-format.md)
...
14. [S13-14 — Atomic file write (`writeTextAtomic`, `writeBytesAtomic`)](../../done/S13-14-atomic-write.md)
