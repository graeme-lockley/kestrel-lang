...
The `kestrel:http` module currently only exports `nowMs()`. The spec (02) requires `createServer`, `listen`, `get`, `bodyText`, `queryParam`, `requestId`, and `nowMs`. Most of these require async I/O and the event loop (sequence **55**).
...
- Prior work in `docs/kanban/done/45-stdlib-json-fs-http.md` may note partial HTTP deferral.

## Dependencies

- Sequence **55** (Async/Await event loop) is effectively a prerequisite for `listen`, `get`, `bodyText`.
