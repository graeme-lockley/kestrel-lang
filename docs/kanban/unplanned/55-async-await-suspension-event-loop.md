...
This is a large feature that is deferred until the language and VM test story (sequence **13**) and core correctness work are in good shape. Full async is needed for the HTTP server (sequence **56**) and concurrent I/O scenarios.

## Dependencies

- Sequences **12**–**15** (VM stack guard, VM integration tests, overflow/divzero tests) reduce risk before large VM changes.
