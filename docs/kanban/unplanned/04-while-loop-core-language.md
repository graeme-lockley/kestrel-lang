# While loop core language support

## Sequence: 04
## Tier: 7 — Deferred (large / dependency-heavy)
## Former ID: (unplanned)

## Summary

Add a first-class `while` loop to the language so iterative algorithms can run without recursion. This directly reduces stack-overflow pressure by replacing recursion-as-loop patterns with explicit iteration.

## Motivation

- Users currently write recursion to model simple loops.
- Without TCO everywhere, recursion-based loops can overflow the stack.
- A native `while` improves clarity and performance for stateful iteration.

## Current State

- No `while` keyword in lexer/parser grammar.
- No loop AST node and no dedicated loop codegen path.
- Specs do not define `while` syntax or semantics.

## Design

- Syntax: `while (Expr) Block` (exact delimiter choices to align with existing grammar style).
- Condition must type-check as `Bool`.
- Loop expression/statement result is `Unit`.
- Runtime semantics: evaluate condition before each iteration; execute body while true.

## Acceptance Criteria

- [ ] Lexer recognizes `while` keyword.
- [ ] Parser adds `while` statement/expression form in appropriate block positions.
- [ ] AST includes a loop node with condition and body.
- [ ] Type checker enforces boolean condition and `Unit` result behavior.
- [ ] Codegen (VM and JVM) lowers to condition-check + back-edge jump structure.
- [ ] Add end-to-end tests showing recursion-based loops can be rewritten as `while` without stack growth.
- [ ] Update specs for grammar, typing, and runtime semantics.

## Test Expectations

- [ ] Parser tests for valid/invalid `while` forms.
- [ ] Typecheck tests for non-boolean condition errors.
- [ ] Runtime tests: counting loops, zero-iteration loops, and nested loops.
- [ ] Runtime tests: large iteration counts complete without stack overflow.

## Spec References

- 01-language (statement/expression grammar and semantics)
- 06-typesystem (condition typing and loop result type)
- 04-bytecode-isa (jump layout for loop lowering)
- 05-runtime-model (execution model for iterative control flow)
