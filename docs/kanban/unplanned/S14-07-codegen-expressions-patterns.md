...
dispatch, and `try/catch/throw` — to `stdlib/kestrel/compiler/codegen.ks`.
...
1. Create `stdlib/kestrel/compiler/codegen.ks` with:
...
- `stdlib/kestrel/compiler/codegen.ks` compiles without errors (may export stubs for decl-level
...
- A test file `stdlib/kestrel/compiler/codegen-expr.test.ks` covers representative expression
  types (literals, arithmetic, match, record access) using the snapshot/golden approach.
- `./kestrel test stdlib/kestrel/compiler/codegen-expr.test.ks` passes.
