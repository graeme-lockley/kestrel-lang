...
- **59 - Async/Await / event loop:** Prerequisite for `listen`, `get`, and `bodyText` (`Task`-returning APIs). This story should not start implementation until 59's model for scheduling blocking I/O is settled enough to hang TCP accept/read/write on.
- **68-70 (Tier 8 - networking expansion):** Roadmap items include `kestrel:socket`, richer TLS usage, "REST-capable HTTP **client** extensions," and possible `kestrel:web` routing. **60** still targets a **minimal** HTTP/1.1 stack (no HTTP/2, no generic socket API, no connection pooling) but **does** include **HTTPS for `get`** on the client and **HTTP-only** for `listen`/`createServer`. Tier 8 can add sockets, pooling, broader TLS features, and routing without redefining the core `kestrel:http` entry points unless specs are updated.
...
- **URL and parsing:** `get` URL parsing may overlap with **62 (URL stdlib)** if strict URL types are introduced later; for 60, a minimal string-based parser in VM or Kestrel is acceptable if documented.
...
|------|----------------------------------|--------|------|
| **Prerequisite** | [Epic E01 — Async Runtime Foundation](../epics/unplanned/E01-async-runtime-foundation.md) (stories S01-01 through S01-06) | **S03-01 starts only after Epic E01** delivers real `Task` runtime, virtual thread executor, non-blocking I/O, and typed Result errors | **Blocker** if ignored |
...
- [ ] [docs/specs/05-runtime-model.md](../../specs/05-runtime-model.md) - How HTTP I/O registers with the event loop (if not already implied by **59**); server concurrency model.
...
- **`bodyText` / `get` errors:** Follow **59**'s **`Task<Result<...>>`** (or documented equivalent) for network/TLS failures; avoid reintroducing empty-string sentinels for new surfaces.
...
- **Relationship to 68-70:** No new public module names required for **60**; Tier 8 later adds `kestrel:socket`, pooling, etc., without renaming core `kestrel:http` entry points unless specs change.
