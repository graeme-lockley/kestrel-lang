...
- `logical_not.ks` — expected output was updated to match wrong VM behaviour
- `unary_mixed.ks` — uses `!(3 > 5)` and `!(False | False)`; expected was updated to match wrong VM behaviour

## Acceptance Criteria

- [ ] Identify and fix root cause (compiler or VM)
- [ ] Restore correct expected output in `tests/e2e/expected/logical_not.stdout`:
  ```
  false
  true
  true
  ...
  100
  ```
- [ ] Restore correct expected output in `tests/e2e/expected/unary_mixed.stdout`:
  ```
  5
  -16
  -15
