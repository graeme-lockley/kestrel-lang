...
3. Run or confirm the story’s required suites: at minimum per **AGENTS.md** (`compiler` tests, `./scripts/kestrel test`, `vm` tests when relevant; E2E/conformance if the story requires).
4. Verify **Documentation and specs to update** items are done or explicitly deferred with a tracked follow-up.
5. Report: **pass** (safe to move to `done/`) or **fail** with concrete gaps—do not move the file unless the user instructs you to after fixes.
