...
export fun toInt(s: String): Option<Int> = {
  val t = trim(s)
  if (length(t) == 0) None
  else if (equals(slice(t, 0, 1), "-")) {
    if (length(t) == 1) None
    else if (isAllDigitsUnsigned(t, 1)) Some(parseInt(t)) else None
  } else if (isAllDigitsUnsigned(t, 0)) Some(parseInt(t)) else None
}

export fun fromInt(n: Int): String = "${n}"
...
export fun repeat(n: Int, s: String): String = repStr(n, s, "")
...
export fun lines(s: String): List<String> = split(s, "\n")
...
fun wordsSkip(s: String, i: Int): List<String> =
  if (i >= length(s)) []
  else if (isAsciiWsCp(codePointAt(s, i))) wordsSkip(s, i + 1)
  else wordsTok(s, i, i)

fun wordsTok(s: String, start: Int, i: Int): List<String> =
  if (i >= length(s)) [slice(s, start, i)]
  else if (isAsciiWsCp(codePointAt(s, i))) slice(s, start, i) :: wordsSkip(s, i + 1)
  else wordsTok(s, start, i + 1)

export fun words(s: String): List<String> = wordsSkip(s, 0)

export fun contains(needle: String, haystack: String): Bool = indexOf(haystack, needle) >= 0
...
fun revInts(xs: List<Int>, acc: List<Int>): List<Int> = match (xs) {
  [] => acc
  h :: t => revInts(t, h :: acc)
}
...
export fun indexes(needle: String, haystack: String): List<Int> = indexesLoop(haystack, needle, 0, [])
export fun indices(needle: String, haystack: String): List<Int> = indexes(needle, haystack)

export fun mapChars(s: String, f: (Char) -> Char): String = mapCharLoop(s, 0, length(s), "", f)
...
export fun filterChars(s: String, pred: (Char) -> Bool): String = filterCharLoop(s, 0, length(s), "", pred)
...
fun trimLeftIdx(s: String, i: Int): Int =
  if (i >= length(s)) i
  else if (isAsciiWsCp(codePointAt(s, i))) trimLeftIdx(s, i + 1) else i

fun trimRightIdx(s: String, i: Int): Int =
  if (i <= 0) 0
  else if (isAsciiWsCp(codePointAt(s, i - 1))) trimRightIdx(s, i - 1) else i

export fun trimLeft(s: String): String = slice(s, trimLeftIdx(s, 0), length(s))
export fun trimRight(s: String): String = slice(s, 0, trimRightIdx(s, length(s)))

export fun fromChar(c: Char): String = charStr(c)
export fun cons(c: Char, s: String): String = append(fromChar(c), s)

fun revChars(xs: List<Char>, acc: List<Char>): List<Char> = match (xs) {
  [] => acc
  h :: t => revChars(t, h :: acc)
}

export fun toList(s: String): List<Char> = toListLoop(s, 0, length(s), [])
fun toListLoop(s: String, i: Int, n: Int, acc: List<Char>): List<Char> =
  if (i >= n) revChars(acc, []) else toListLoop(s, i + 1, n, charAt(s, i) :: acc)

export fun fromList(cs: List<Char>): String = match (cs) {
  [] => ""
  h :: t => append(fromChar(h), fromList(t))
}
