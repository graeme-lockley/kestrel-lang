...
export fun staticCss(): String =
  Str.join("\n", [
    ...
    "pre code.kestrel, pre code.language-kestrel, pre code.language-ks { color: #1f2328; }",
    "pre code.kestrel .tok-kw, pre code.language-kestrel .tok-kw, pre code.language-ks .tok-kw { color: #005cc5; font-weight: 600; }",
    "pre code.kestrel .tok-type, pre code.language-kestrel .tok-type, pre code.language-ks .tok-type { color: #6f42c1; }",
    "pre code.kestrel .tok-lit, pre code.language-kestrel .tok-lit, pre code.language-ks .tok-lit { color: #22863a; }",
    "pre code.kestrel .tok-comment, pre code.language-kestrel .tok-comment, pre code.language-ks .tok-comment { color: #6a737d; font-style: italic; }",
    "pre code.kestrel .tok-op, pre code.language-kestrel .tok-op, pre code.language-ks .tok-op { color: #d73a49; }",
    "pre code.kestrel .tok-punct, pre code.language-kestrel .tok-punct, pre code.language-ks .tok-punct { color: #24292e; }",
    ...
    ".decl-index { position: sticky; top: 56px; max-height: calc(100vh - 72px); overflow-y: auto; overflow-x: auto; border: 1px solid #dde; border-radius: 6px; padding: 0.8rem; background: #fafafe; }",
    ".decl-index-title { font-weight: 600; font-size: 0.9rem; margin-bottom: 0.5rem; color: #444; }",
    ".decl-index ul { list-style: none; padding: 0; margin: 0; min-width: max-content; }",
