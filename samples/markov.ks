...
fun observe(model: Dict<String, Dict<String, Int>>, ctx: String, ch: String): Dict<String, Dict<String, Int>> = {
...
fun trainLoop(model: Dict<String, Dict<String, Int>>, a: String, b: String, c: String, chars: List<String>): Dict<String, Dict<String, Int>> =
  match (chars) {
    [] =>
      model,
    h :: t =>
      trainLoop(observe(model, "${a}${b}${c}", h), b, c, h, t)
  }
        ...
        s :: more =>
          match (more) {
            [] =>
              Dict.empty(),
            t :: tail =>
              trainLoop(Dict.empty(), h, s, t, tail)
          }
      }
  }
}
...
fun nextChar(model: Dict<String, Dict<String, Int>>, ctx: String): String =
  match (Dict.get(model, ctx)) {
    None =>
      " ",  // unseen context — emit a space
    Some(d) =>
      sampleFrom(d)
  }

// Generate `n` characters, keeping a sliding 3-char context window.
fun genLoop(model: Dict<String, Dict<String, Int>>, ctx: String, n: Int, acc: String): String =
  if (n <= 0)
    acc
  else {
...
fun insertByCount(p: String * Int, xs: List<String * Int>): List<String * Int> =
  match (xs) {
    [] =>
      [p],
    h :: t =>
      if (p.1 >= h.1) p :: xs else h :: insertByCount(p, t)
  }
...
fun printFreqs(rows: List<String * Int>, maxCount: Int): Unit =
  match (rows) {
    [] =>
      (),
    h :: t => {
      printFreqRow(h, maxCount)
      printFreqs(t, maxCount)
    }
  }
...
fun main(): Unit = {
  ...
  Lst.forEach(seeds, (seed: String) => {val text = generate(model, seed, 76)println("   [${seed}] ${text}")})
  println("")
