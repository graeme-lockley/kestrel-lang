...
// ── Term ADT ─────────────────────────────────────────────────────────────
type Term = Var(String) | Lam(String, Term) | App(Term, Term)

// ── Parse-result ADT ─────────────────────────────────────────────────────
type ParseResult = POk(Term, List<String>) | PErr(String)

// ── Pretty-printer ────────────────────────────────────────────────────────
fun pretty(t: Term): String = match (t) {
  Var(x)    => x
  Lam(x, b) => "(λ${x}.${pretty(b)})"
  App(f, a) => "(${pretty(f)} ${pretty(a)})"
}

// ── Free-variable set (as list, no duplicates) ────────────────────────────
fun hasMember(xs: List<String>, s: String): Bool = match (xs) {
  []     => False
  h :: t => if (h == s) True else hasMember(t, s)
}

fun listUnion(xs: List<String>, ys: List<String>): List<String> = match (ys) {
  []     => xs
  h :: t => if (hasMember(xs, h)) listUnion(xs, t) else listUnion(h :: xs, t)
}

fun listRemove(xs: List<String>, s: String): List<String> = match (xs) {
  []     => []
  h :: t => if (h == s) listRemove(t, s) else h :: listRemove(t, s)
}

fun freeVars(t: Term): List<String> = match (t) {
  Var(x)    => [x]
  Lam(x, b) => listRemove(freeVars(b), x)
  App(f, a) => listUnion(freeVars(f), freeVars(a))
}

// ── Capture-avoiding substitution subst(t, x, s) = t[x := s] ─────────────
fun freshName(name: String, avoid: List<String>): String =
  if (hasMember(avoid, "${name}'")) freshName("${name}'", avoid)
  else "${name}'"

fun subst(t: Term, x: String, s: Term): Term = match (t) {
  Var(y)    => if (y == x) s else Var(y)
...
fun stepNonLam(fn: Term, arg: Term): Option<Term> =
  match (step(fn)) {
    None     => stepArg(fn, arg)
    Some(f2) => Some(App(f2, arg))
  }

// Applies function fn to arg, performing beta reduction if fn is a lambda.
fun applyApp(fn: Term, arg: Term): Option<Term> = match (fn) {
  Lam(lv, lb) => Some(subst(lb, lv, arg))
  _           => stepNonLam(fn, arg)
}

fun step(t: Term): Option<Term> = match (t) {
  Var(_)       => None
  Lam(lv, lb)  => stepLam(lv, lb)
  App(fn, arg) => applyApp(fn, arg)
}
...
fun reduceLoop(key: String, t: Term, limit: Int): Option<Term> =
  if (limit <= 0) None
    ...
    None     => Some(t)
    Some(t2) => reduceLoop(key, t2, limit - 1)
  }

fun reduce(t: Term, limit: Int): Option<Term> = reduceLoop("", t, limit)
...
fun tokenAt(s: String, i: Int, acc: List<String>): List<String> = {
  ...
  if (i >= len) Lst.reverse(acc)
    ...
    if (cp == 32 | cp == 9 | cp == 10 | cp == 13) tokenAt(s, i + 1, acc)
    else if (cp == 92 | cp == 955) tokenAt(s, i + 1, "LAM" :: acc)  // \ or λ
    else if (cp == 46)             tokenAt(s, i + 1, "DOT" :: acc)   // .
    else if (cp == 40)             tokenAt(s, i + 1, "LPAREN" :: acc) // (
    else if (cp == 41)             tokenAt(s, i + 1, "RPAREN" :: acc) // )
    else if (cpIsAlpha(cp)) {
      val name = readIdent(s, i, "")
      tokenAt(s, i + Str.length(name), "VAR:${name}" :: acc)
    }
    else tokenAt(s, i + 1, acc)
  }
}

fun tokenize(s: String): List<String> = tokenAt(s, 0, [])
...
fun varName(tok: String): Option<String> =
  if (Str.length(tok) >= 4 & Str.slice(tok, 0, 4) == "VAR:")
    Some(Str.slice(tok, 4, Str.length(tok)))
  else None

fun isAtomStart(tokens: List<String>): Bool = match (tokens) {
  []     => False
  h :: _ =>
    h == "LPAREN" | h == "LAM" |
    (Str.length(h) >= 4 & Str.slice(h, 0, 4) == "VAR:")
}

// parseParenClose: check for RPAREN after a successfully parsed sub-term.
fun parseParenClose(t: Term, rest2: List<String>): ParseResult = match (rest2) {
  []            => PErr("missing ')'")
  tok2 :: rest3 =>
    if (tok2 == "RPAREN") POk(t, rest3)
    else PErr("expected ')'")
}

// parseParenResult: handle the outcome of parsing the inner term of ( term ).
fun parseParenResult(r: ParseResult): ParseResult = match (r) {
  PErr(msg)     => PErr(msg)
  POk(t, rest2) => parseParenClose(t, rest2)
}

// parseAtomVar: parse a variable token.
fun parseAtomVar(tok: String, rest: List<String>): ParseResult =
  match (varName(tok)) {
    None       => PErr("unexpected token: ${tok}")
    Some(name) => POk(Var(name), rest)
  }

// parseLamBodyResult: wrap result of parsing a lambda body.
fun parseLamBodyResult(name: String, r: ParseResult): ParseResult = match (r) {
  PErr(msg)        => PErr(msg)
  POk(body, rest3) => POk(Lam(name, body), rest3)
}

// parseLamBody: expect "." then term.
fun parseLamBody(name: String, rest: List<String>): ParseResult = match (rest) {
  []           => PErr("missing '.' after λ${name}")
  dot :: rest2 =>
    if (dot != "DOT") PErr("expected '.' after λ${name}")
    else parseLamBodyResult(name, parseTerm(rest2))
}

// parseLamVar: match the variable token after λ.
fun parseLamVar(varOpt: Option<String>, rest: List<String>): ParseResult =
  match (varOpt) {
    None       => PErr("expected identifier after λ")
    Some(name) => parseLamBody(name, rest)
  }

// parseLam: parse \VAR.term
fun parseLam(rest: List<String>): ParseResult = match (rest) {
  []           => PErr("missing variable after λ")
  tok :: rest2 => parseLamVar(varName(tok), rest2)
}

// parseAtom: parse a single atomic term.
fun parseAtom(tokens: List<String>): ParseResult = match (tokens) {
  [] => PErr("unexpected end of input")
  tok :: rest =>
    if (tok == "LPAREN") parseParenResult(parseTerm(rest))
    else if (tok == "LAM") parseLam(rest)
    else parseAtomVar(tok, rest)
}

fun parseTerm(tokens: List<String>): ParseResult = parseApp(tokens)

fun parseApp(tokens: List<String>): ParseResult =
  match (parseAtom(tokens)) {
    PErr(msg)    => PErr(msg)
    POk(f, rest) => parseAppCont(f, rest)
  }
...
fun parseAppCont(f: Term, tokens: List<String>): ParseResult =
  if (isAtomStart(tokens))
    match (parseAtom(tokens)) {
      PErr(msg)    => PErr(msg)
      POk(a, rest) => parseAppCont(App(f, a), rest)
    }
  else POk(f, tokens)

// parseEnd: verify no trailing tokens remain.
fun parseEnd(t: Term, rest: List<String>): ParseResult = match (rest) {
  [] => POk(t, [])
  _  => PErr("trailing input after expression")
}

fun parseTermResult(r: ParseResult): ParseResult = match (r) {
  PErr(msg)    => PErr(msg)
  POk(t, rest) => parseEnd(t, rest)
}

fun parse(input: String): ParseResult = parseTermResult(parseTerm(tokenize(input)))
...
fun demo(title: String, source: String): Unit = {
  ...
  match (parse(source)) {
    PErr(msg) => println("  error:  ${msg}")
    POk(t, _) => {
      println("  parsed: ${pretty(t)}")
      match (reduce(t, 500)) {
        None    => println("  nf:     (diverges or exceeds 500 steps)")
        Some(r) => println("  nf:     ${pretty(r)}")
      }
    }
  }
}
