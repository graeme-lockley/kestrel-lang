...
fun tokenAt(s: String, i: Int, acc: List<String>): List<String> = {
  ...
  if (i >= len) 
    ...
    if (cp == 32 | cp == 9 | cp == 10 | cp == 13) 
      ...
      tokenAt(s, i + 1, "LAM" :: acc)  // \ or λ
    else if (cp == 46) 
      tokenAt(s, i + 1, "DOT" :: acc)   // .
    else if (cp == 40) 
...
fun parseLamBody(name: String, rest: List<String>): ParseResult = 
  match (rest) {
    ...
    dot :: rest2 =>
      if (dot != "DOT") 
        PErr("expected '.' after λ${name}")
      else 
        parseLamBodyResult(name, parseTerm(rest2))
  }
