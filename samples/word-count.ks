...
fun tally(freq: Dict<String, Int>, w: String): Dict<String, Int> = {
  val word = clean(w)
  if (Str.isEmpty(word)) freq
  else match (Dct.get(freq, word)) {
    None    => Dct.insert(freq, word, 1)
    Some(n) => Dct.insert(freq, word, n + 1)
  }
}
...
fun insertDesc(pair: (String, Int), xs: List<(String, Int)>): List<(String, Int)> =
  match (xs) {
    []     => [pair]
    h :: t => if (pair.1 >= h.1) pair :: (h :: t) else h :: insertDesc(pair, t)
  }

fun sortDesc(xs: List<(String, Int)>): List<(String, Int)> = match (xs) {
  []     => []
