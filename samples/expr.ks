...
fun pretty(e: Expr): String = match (e) {
  Lit(n)    => Str.fromInt(n)
  Add(l, r) => "(${pretty(l)} + ${pretty(r)})"
  Sub(l, r) => "(${pretty(l)} - ${pretty(r)})"
  Mul(l, r) => "(${pretty(l)} * ${pretty(r)})"
  Neg(e)    => "(-${pretty(e)})"
}
...
show("towers", Mul(
  Sub(Mul(Add(Lit(2), Lit(3)), Lit(4)), Mul(Lit(6), Lit(2))),
  Add(Sub(Lit(7), Lit(1)), Mul(Lit(2), Lit(2)))
))
