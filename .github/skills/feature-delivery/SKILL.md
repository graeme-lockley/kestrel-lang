...
Use this workflow for **kanban-driven** or **spec-driven** features in this repo. It complements [AGENTS.md](AGENTS.md), and the kanban skills **kanban-story-create** / **kanban-story-migrate**.
...
6. **Kanban** — **Planned → doing:** move the file to `docs/kanban/doing/`, add or keep **Build notes**, tick **Tasks** as you go; add tasks if scope grows. **Doing → done:** all tasks `[x]`, tests green → move to `docs/kanban/done/` (see **kanban-story-migrate**). Do not skip **planned** without explicit team agreement.
...
- If building a kestrel library, the tests must be alongside the library file, otherwise the bulk of the tests must be the unit tests in `tests/unit`
